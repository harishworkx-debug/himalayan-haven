...
function RoomsPage() {
  ...
  return <><PageHero eyebrow="Rooms at Aashiyana" title="Comfort, with the Himalayas close by." copy="Simple, clean rooms for a restful stay in Sangla. Message us for current room availability and seasonal pricing." image={images.windowRoom} imageAlt="Comfortable room with wide mountain-facing windows" /><section className="py-18 lg:py-28"><div className="mx-auto max-w-7xl px-5 lg:px-8"><SectionHeading eyebrow="Stay your way" title="Rooms shown as they are." copy="We keep room information honest and straightforward. The photographs below are from Aashiyana Guest House; room assignment depends on availability." /><div className="mt-12 grid gap-8 lg:grid-cols-3">{rooms.map((room) => <article key={room.title} className="overflow-hidden rounded-md border border-border bg-card"><img src={room.image} alt={room.alt} className="aspect-[4/5] size-full object-cover transition-transform duration-700 hover:scale-[1.03]" /><div className="p-6"><h2 className="font-display text-3xl">{room.title}</h2><p className="mt-3 text-sm leading-7 text-muted-foreground">{room.copy}</p><Button asChild variant="outline" className="mt-6 w-full"><a href={whatsappUrl} target="_blank" rel="noreferrer"><MessageCircle /> Enquire about this room</a></Button></div></article>)}</div><p className="mt-10 text-center text-sm text-muted-foreground">Stays start around ₹1,611 per night. Pricing and availability vary by season.</p></div></section><FinalCta /></>;
}